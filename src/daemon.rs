...
impl Daemon {
    ...
    pub fn status(&self) -> crate::Result<()> {
        ...
        for (raw_pid, process) in rows {
            let manager = manager_label(process, pidfile_pid);
            println!(
                "{:<8} {:<10} {:<8.1} {:<10.1} {:<8}",
                raw_pid,
                manager,
...
/// On Linux, tasks under `/proc/.../task/` can appear as separate PIDs; `Process::thread_kind()`
/// is `Some` for those entries. We keep only rows where `thread_kind()` is `None` (not a per-task
/// row). On other platforms sysinfo always returns `None` for `thread_kind`, so nothing is
/// filtered.
#[inline]
...
/// `daemon` if this PID matches the pidfile; otherwise `direct`.
fn manager_label(proc: &sysinfo::Process, pidfile_pid: Option<u32>) -> &'static str {
